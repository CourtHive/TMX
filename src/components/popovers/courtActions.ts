...
export const courtActions = () => (e: MouseEvent, cell: any): void => {
  ...
  const doneEditing = ({ success }: any) => {
    if (success) {
      ...
      for (const venue of venues) {
        const court = venue.courts?.find((c: any) => c.courtId === data.courtId);
        if (court) {
          updatedCourt = court;
          break;
        }
  ...
  const items = [
    {
      onClick: () => editCourt({ court: data, callback: doneEditing }),
      text: 'Edit',
    },
  ];
